 ...
 *
 * @typedef GrWorldAutoUIOptions
 * @property {Object} [where] - default container for object/world UI for this world
 * @property {number} [width=300] - default panel width
 * @property {number} [widthdiv=1] - panel width subdivision
 * @property {boolean} [adjusted=false] - slider label adjustment setting
 ...
 *
 * `canvas-overlay` anchors lil-gui to the renderer's containing element. This is
 * intentionally container-relative (not always strict canvas-pixel-relative).
 * For layouts where a container holds multiple canvases (for example side-by-side
 * views), the panel appears at that container's top-right corner for each world.
 * If strict per-canvas overlay is desired, place each world canvas in its own
 * dedicated container (or set `where` accordingly).
 */
 ...
 * @property [lightBrightness=2.25] - brightness of the default lights (was .75, converted to physical units)
 ...
 * @property {{checked:boolean}} [runbutton] - a checkbox-like object to switch things on or off
 * @property {{value:(number|string)}} [speedcontrol] - a slider-like object to get the speed
 */
...
export class GrWorld {
    ...
    constructor(params = {}) {
        ...
        if (!("lightBrightness" in params)) params.lightBrightness = 2.25;
        ...
        if (this.active_camera.isPerspectiveCamera) {
            ...
            let flySaveState = function () {
                /** @type {any} */
                const fc = this;
                fc.position0 = new T.Vector3(
                    fc.object.position.x,
                    fc.object.position.y,
                    fc.object.position.z
                );
            };
        ...
        let soloDirLight = new T.DirectionalLight(0xf8f8ff, 1.0);
        this.solo_scene.add(soloDirLight);
        ...
        this.chkSolo = undefined;
        /** @type {HTMLSelectElement|undefined} */
        this.selectViewMode = undefined;
        /** @type {HTMLSelectElement|undefined} */
        this.selectRideable = undefined;
        /** @type {HTMLSelectElement|undefined} */
        this.selectLook = undefined;

        /** @type {GrWorldAutoUIOptions} */
        this.autoUIOptions = {
            where: "where" in params ? params.where : document.body,
            width: 300,
            widthdiv: 1,
            adjusted: false,
            useLilGUI: true,
        ...
        this.autoUIGUI = undefined;
    } // end of constructor

    /**
     * Set defaults for AutoUI controls in this world.
     *
     * @param {GrWorldAutoUIOptions} options
     */
    setAutoUIOptions(options = {}) {
        this.autoUIOptions = Object.assign({}, this.autoUIOptions, options);
    }

    /**
     * Get the AutoUI defaults for this world.
     *
     * @returns {GrWorldAutoUIOptions}
     */
    getAutoUIOptions() {
        return this.autoUIOptions;
    }
